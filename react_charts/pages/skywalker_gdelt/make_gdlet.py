...
def get_and_save_gdelt_articles(query, output_file=None, max_records=100, sort="datedesc"):
    """
    Get article data from GDELT API using JSON format, process it, and save to a Parquet file
    
    Parameters:
    query (str): Search query (e.g., "yale endowment")
    ...
    # Construct URL - using JSON format like the library
    url = f"https://api.gdeltproject.org/api/v2/doc/doc?format=json&timespan=FULL&query={encoded_query}&mode=artlist&maxrecords={max_records}&sort={sort}"
    ...
    if output_file is None:
        ...
        output_file = f"{query.replace(' ', '_').lower()}_articles.parquet"
...
# Example usage for "harvard endowment"
if __name__ == "__main__":
    query = "harvard endowment"
    articles_df = get_and_save_gdelt_articles(query, max_records=250)
...
def get_and_save_gdelt_articles(query, output_file=None, max_records=250, sort="datedesc", 
                         ...
                         domain=None, country=None, language=None):
    """
    Get article data from GDELT API using JSON format, process it, and save to a Parquet file
...
if __name__ == "__main__":
    ...
    # Example with domain filter - only articles from specific sources
    ...
    # Example with date range and country filter
